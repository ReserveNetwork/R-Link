...
# this returns a file path based on if we are running rlink.py directly, or if we have packed it as an exe with cxfreeze
...
class RLink:
    ...
    def __init__(self, identity: RNS.Identity, storage_dir, reticulum_config_dir):
        ...
        # register location announce identity
        self.location_manager = RNS.Destination(
            self.identity,
            RNS.Destination.IN,
    ...
    def run(self, host, port, launch_browser: bool):
        ...
        async def index(request):
            ...
            video_field = None
            if "video" in fields:
                audio_bytes = base64.b64decode(data["lxmf_message"]["fields"]["video"]["audio_bytes"])
                video_bytes = base64.b64decode(data["lxmf_message"]["fields"]["video"]["video_bytes"])
                video_field = LxmfVideoField(audio_bytes, video_bytes)
    ...
    async def update_config(self, data):
        ...
        if "telemetry_enabled" in data:
            value = bool(data["telemetry_enabled"])
            self.config.telemetry_enabled.set(value)

        if "location" in data and data["location"] != "":
            self.config.location.set(data["location"])
    ...
    async def on_websocket_data_received(self, client, data):
        ...
        # handle downloading a page from a nomadnet node
            ...
            # download the page
            downloader = NomadnetPageDownloader(destination_hash, page_path_to_download, page_data,
    ...
    def get_config_dict(self):
        return {
            ...
            "telemetry_enabled": self.config.telemetry_enabled.get(),
            "location": self.config.location.get(),
        }
    ...
    def convert_lxmf_message_to_dict(self, lxmf_message: LXMF.LXMessage):
        ...
        for field_type in message_fields:
            ...
            if field_type == LXMF.FIELD_CUSTOM_TYPE:
                ...
                video_bytes = base64.b64encode(value[1]).decode("utf-8")
                fields["video"] = {
                    "audio_bytes": audio_bytes,
                    "video_bytes": video_bytes,
                }
    ...
    async def send_message(self, destination_hash: str, content: str,
        ...
        if video_field is not None:
            lxmf_message.fields[LXMF.FIELD_CUSTOM_TYPE] = [
                video_field.audio_bytes,
                video_field.video_bytes,
            ]
    ...
    # handle an announce received from reticulum, for a location
    # NOTE: cant be async, as Reticulum doesn't await it
    def on_location_announce_received(self, aspect, destination_hash, announced_identity, app_data):

        # log received announce
        print("Received an announce from " + RNS.prettyhexrep(destination_hash) + " for [location]")
    ...
    def parse_location(self, app_data_base64: str):
        try:
            app_data_bytes = base64.b64decode(app_data_base64)
            return app_data_bytes.decode("utf-8")
        except:
...
class Config:
    ...
    telemetry_enabled = BoolConfig("telemetry_enabled", True)
    location = StringConfig("location", None)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="RLink")
    ...
    # init app
    rlink = RLink(identity, args.storage_dir, args.reticulum_config_dir)
    rlink.run(args.host, args.port, launch_browser=args.headless is False)
