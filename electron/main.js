...
app.whenReady().then(async () => {
    ...
    // find path to python/cxfreeze reserve network rlink executable
    const exeName = process.platform === "win32" ? "RLink.exe" : "RLink";
    ...
    try {
        ...
        const storageDir = path.join(app.getPath('home'), 'r-link'); // ~/.r-link

        // migrate old storage dir to new storage dir
        const oldStorageDir = path.join(app.getPath('home'), '.r-link-webchat'); // ~/.r-link-webchat
        ...
        const requiredArguments = [
            '--headless', // rlink usually launches default web browser, we don't want this when using electron
