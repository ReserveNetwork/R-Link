  ...
  <div class="flex flex-col w-80 min-w-80">
    ...
    <div class="bg-white border-b border-r border-gray-200">
      <div class="-mb-px flex">
        <div @click="tab = 'conversations'"
             class="w-full border-b-2 py-3 px-1 text-center text-sm font-medium cursor-pointer"
             :class="[ tab === 'conversations' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-700']">
          Conversations
        </div>
        <div @click="tab = 'announces'"
             class="w-full border-b-2 py-3 px-1 text-center text-sm font-medium cursor-pointer"
             :class="[ tab === 'announces' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-700']">
          Announces
        </div>
      </div>
    </div>
    ...
    <div v-if="tab === 'conversations'" class="flex-1 flex flex-col bg-white border-r overflow-hidden">
      ...
      <div v-if="conversations.length > 0" class="p-1 border-b border-gray-300">
        <input v-model="conversationsSearchTerm" type="text"
      ...
      <div class="flex h-full overflow-y-auto">
        <div v-if="searchedConversations.length > 0" class="w-full">
          <div @click="onConversationClick(conversation)" v-for="conversation of searchedConversations"
            ...
            <div class="mr-auto">
              <div class="text-gray-900"
                   :class="{ 'font-semibold': conversation.is_unread || conversation.failed_messages_count > 0 }">
    ...
    <div v-if="tab === 'announces'" class="flex-1 flex flex-col bg-white border-r overflow-hidden">
      ...
      <div v-if="peersCount > 0" class="p-1 border-b border-gray-300">
        <input v-model="peersSearchTerm" type="text" :placeholder="`Search ${peersCount} recent announces...`"
               class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
      </div>
      ...
      <div class="flex h-full overflow-y-auto">
        <div v-if="searchedPeers.length > 0" class="w-full">
          <div @click="onPeerClick(peer)" v-for="peer of searchedPeers" class="flex cursor-pointer p-2 border-l-2"
               :class="[ peer.destination_hash === selectedDestinationHash ? 'bg-gray-100 border-blue-500' : 'bg-white border-transparent hover:bg-gray-50 hover:border-gray-200' ]">
            <div class="my-auto mr-2">
